...
struct Rigid
{
    ...
    Rigid(Solver* solver, float3 size, float density, float friction, float3 position, quat rotation, float3 velocity = float3{ 0, 0, 0 }, Shape shape = Shape::Box, bool isStatic = false);
    ~Rigid();
...
{    
    union FeaturePair
    {
        ...
        int key; // trick for fast comparison
    };
