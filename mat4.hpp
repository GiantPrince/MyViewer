...
static_assert(sizeof(mat4) == 16 * sizeof(float), "mat4 is exactly 16 floats");

using vec4 = std::array<float, 4>;
static_assert(sizeof(vec4) == 4 * sizeof(float), "vec4 is exactly 4 floats");
...
inline mat4 perspective(float vfov, float aspect, float near, float far) {
	const float e = 1 / std::tan(vfov / 2);
