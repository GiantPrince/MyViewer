...
void Helpers::transfer_to_cubemap(void const* data, size_t size, AllocatedImage& image)
{
	...
	assert(size == image.extent.height * image.extent.width * 6 * bytes_per_block / texels_per_block);
	...
	for (uint32_t i = 0; i < 6; i++) {
		VkBufferImageCopy region{
			.bufferOffset = i * image.extent.width * image.extent.height,
...
VkFormat Helpers::find_image_format(std::vector< VkFormat > const &candidates, VkImageTiling tiling, VkFormatFeatureFlags features) const {
	for (VkFormat format : candidates) {
		...
		vkGetPhysicalDeviceFormatProperties(rtg.physical_device, format, &properties);
...
void Helpers::create() {
	...
	VK(vkCreateCommandPool(rtg.device, &pool_info, nullptr, &transfer_command_pool));
