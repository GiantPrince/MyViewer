...
void Viewer::ShadowMapPipeline::create(RTG& rtg, VkRenderPass render_pass, uint32_t subpass) {
...

	{
		...
		VK(vkCreateDescriptorSetLayout(rtg.device, &create_info, nullptr, &set1_Lights));
	}
...
void Viewer::ShadowMapPipeline::destroy(RTG& rtg) {
	...
	if (set1_Lights != VK_NULL_HANDLE) {
		vkDestroyDescriptorSetLayout(rtg.device, set1_Lights, nullptr);
		set1_Lights = VK_NULL_HANDLE;
	}
